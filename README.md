...
- **Der Deploy läuft auf versionierten Migrationen.** `start:deploy` ruft
  `db:deploy` (`lib/db/deploy.mjs`): es liest aus jeder Migration, welche
  Tabellen und Spalten sie anlegt, prüft in der Datenbank nach, welche davon
  schon stehen, und markiert **nur nachweislich vollzogene** Migrationen als
  Basislinie — der Rest läuft. `0000` bleibt dabei unverändert (es enthält
  `CREATE TABLE` ohne `IF NOT EXISTS`). Ein Teilstand führt zum Abbruch mit
  Begründung statt zu einer Vermutung. Neuinstallation und Upgrade laufen in
  CI gegen ein echtes Postgres, auf demselben Weg wie der Deploy, und werden
  gegen `drizzle-kit push` aus demselben Schema verglichen — 260 Spalten,
  deckungsgleich ([Details](lib/db/migrations/README.md)). Was bleibt: auf der
  Produktionsdatenbank ist die Umstellung noch nicht gelaufen, der erste
  scharfe Lauf passiert beim nächsten Deploy.
...
- **Die OpenAPI-Spezifikation deckt nur einen Teil ab — aber sie driftet nicht mehr.**
  `lib/api-spec/openapi.yaml` beschreibt 28 Operationen, der Server hat 76
  Route-Handler; 48 bleiben undokumentiert. Sechs Dashboard-Seiten benutzen den
  daraus erzeugten typisierten Client, alle übrigen (Freigaben, Meldungen, MCP,
  Telefon, Vorschläge, Kennzahlen, Zugänge, Startseite) rufen mit `fetch` direkt
  auf — dort gibt es zwischen Server und Oberfläche keine geprüfte Zusage.
  Erzwungen wird seit `scripts/check-openapi.mjs` (in `npm run typecheck`, also
  auch in CI) zweierlei: die Spec darf **keine Route versprechen, die der Server
  nicht hat** — das ist die Richtung, die den erzeugten Client zur Laufzeit auf
  404 laufen lässt —, und die Zahl der undokumentierten Routen darf **nicht
  wachsen**. Eine neue Route ohne Spec-Eintrag macht die Prüfung rot. Die Lücke
  ist damit nicht geschlossen, aber sie kann sich nicht mehr vergrößern.
